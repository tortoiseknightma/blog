...
describe('GitHub Pages branding', () => {
  ...
  const siteDataApi = fs.readFileSync(
    path.join(process.cwd(), 'lib', 'db', 'SiteDataApi.js'),
    'utf8'
  )
  ...
  it('uses the requested site description and two-line sidebar signature', () => {
    ...
    expect(siteDataApi).toMatch(
      /SIMPLE_LOGO_DESCRIPTION:\s*process\.env\.NEXT_PUBLIC_THEME_SIMPLE_LOGO_DESCRIPTION/
    )
  })
})
