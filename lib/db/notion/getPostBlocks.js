...
// ⚠️ 全局限流器（非常关键）
const notionApiConcurrency = Number(process.env.NOTION_API_CONCURRENCY || 1)
const limit = pLimit(
  Number.isFinite(notionApiConcurrency) && notionApiConcurrency > 0
    ? notionApiConcurrency
    : 1
)

// ⚠️ 每个请求之间的间隔（防 burst）
const notionApiInterval = Number(process.env.NOTION_API_INTERVAL_MS || 700)
const REQUEST_INTERVAL =
  Number.isFinite(notionApiInterval) && notionApiInterval >= 0
    ? notionApiInterval
    : 700
...
function getRetryDelay(error, retryAttempts) {
  if (!isRateLimitError(error)) {
    return 1000
  }
  const retryIndex = 3 - retryAttempts
  return Math.min(12000, 3000 * Math.max(1, retryIndex + 1))
}
...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  ...
  try {
    ...
    const retryDelay = getRetryDelay(e, retryAttempts)
    console.warn(
      '[API retry delay]',
      `from:${from}`,
      `id:${id}`,
