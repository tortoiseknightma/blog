  ...
  const ensureBlockMap = async (post) => {
    ...
    try {
      ...
      if (!rawBlockMap?.block) {
        console.warn(
          `[${taskId}] [resolvePostProps] empty Notion blockMap:`,
          post.id
        )
        return post
      }
...
function handleDataBeforeReturn(db) {
  applyExportedNotionConfigOverrides(db)
  ...
  sanitizeExportedSiteImages(db)

  return db
}

function applyExportedNotionConfigOverrides(db) {
  if (process.env.EXPORT !== 'true') return

  db.NOTION_CONFIG = db.NOTION_CONFIG || {}
  db.customNav = db.customNav || []

  const plainOverrides = {
    CONTACT_HOME: process.env.NEXT_PUBLIC_CONTACT_HOME,
    CONTACT_GITHUB: process.env.NEXT_PUBLIC_CONTACT_GITHUB,
    DESCRIPTION: process.env.NEXT_PUBLIC_DESCRIPTION
  }

  Object.entries(plainOverrides).forEach(([key, value]) => {
    if (value) {
      db.NOTION_CONFIG[key] = value
    }
  })

  if (process.env.NEXT_PUBLIC_CONTACT_EMAIL) {
    db.NOTION_CONFIG.CONTACT_EMAIL = btoa(
      unescape(encodeURIComponent(process.env.NEXT_PUBLIC_CONTACT_EMAIL))
    )
  }

  if (process.env.NEXT_PUBLIC_CONTACT_HOME) {
    db.customNav = [
      {
        icon: 'fas fa-home',
        name: '主页',
        href: process.env.NEXT_PUBLIC_CONTACT_HOME,
        target: '_blank',
        show: true
      },
      ...db.customNav.filter(
        item => item?.href !== process.env.NEXT_PUBLIC_CONTACT_HOME
      )
    ]
  }
}

export function sanitizeExportedSiteImages(db) {
  if (process.env.EXPORT !== 'true') return
  ...
  const replaceKnownSiteImage = value => {
    if (typeof value !== 'string') return value
    const isNotionImageUrl =
      /(prod-files-secure|secure\.notion-static\.com|notion\.so\/image)/.test(
        value
      )
    if (!isNotionImageUrl) {
      return value
    }
    if (/Majin\.Archer\.webp/.test(value) && process.env.NEXT_PUBLIC_AVATAR) {
      return process.env.NEXT_PUBLIC_AVATAR
    }
    if (
      /befb1c82323c0ba6a83df6d91b8e48e4af3ac0da9af908de1ad15bb8aa353fe8\.webp/.test(value) &&
      process.env.NEXT_PUBLIC_HOME_BANNER_IMAGE
    ) {
      return process.env.NEXT_PUBLIC_HOME_BANNER_IMAGE
    }
    return value
  }
  ...
  const walk = value => {
    ...
    return replaceKnownSiteImage(value)
  }
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  const exportPageCover =
    process.env.EXPORT === 'true' && process.env.NEXT_PUBLIC_HOME_BANNER_IMAGE
      ? process.env.NEXT_PUBLIC_HOME_BANNER_IMAGE
      : null
  const exportIcon =
    process.env.EXPORT === 'true' && process.env.NEXT_PUBLIC_AVATAR
      ? process.env.NEXT_PUBLIC_AVATAR
      : null
  const exportDescription =
    process.env.EXPORT === 'true' && process.env.NEXT_PUBLIC_DESCRIPTION
  ...
  const defaultPageCover =
    exportPageCover || NOTION_CONFIG?.HOME_BANNER_IMAGE || '/bg_image.jpg'
  const defaultIcon = exportIcon || NOTION_CONFIG?.AVATAR || '/avatar.svg'
  ...
  const link = siteConfig('LINK', defaultLink, NOTION_CONFIG)
