...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  switch (key) {
    ...
    case 'DESCRIPTION':
      // LINK比较特殊，
      if (key === 'LINK') {
        if (process.env.EXPORT === 'true' && process.env.NEXT_PUBLIC_LINK) {
          return convertVal(process.env.NEXT_PUBLIC_LINK)
        }
      ...
      if (
        ...
        return convertVal(process.env.NEXT_PUBLIC_HOME_BANNER_IMAGE)
      }
      if (
        process.env.EXPORT === 'true' &&
        key === 'AVATAR' &&
        ...
        return convertVal(process.env.NEXT_PUBLIC_AVATAR)
      }
      if (
        process.env.EXPORT === 'true' &&
        key === 'CONTACT_HOME' &&
        process.env.NEXT_PUBLIC_CONTACT_HOME
      ) {
        return convertVal(process.env.NEXT_PUBLIC_CONTACT_HOME)
      }
      if (
        process.env.EXPORT === 'true' &&
        key === 'CONTACT_GITHUB' &&
        ...
        return convertVal(process.env.NEXT_PUBLIC_CONTACT_GITHUB)
      }
      if (
        process.env.EXPORT === 'true' &&
        key === 'CONTACT_EMAIL' &&
      ...
      if (
        process.env.EXPORT === 'true' &&
        key === 'POST_LIST_PREVIEW' &&
        process.env.NEXT_PUBLIC_POST_PREVIEW
      ) {
        return convertVal(process.env.NEXT_PUBLIC_POST_PREVIEW)
      }
      if (
        process.env.EXPORT === 'true' &&
        key === 'DESCRIPTION' &&
        ...
        return convertVal(process.env.NEXT_PUBLIC_DESCRIPTION)
      }
      return convertVal(
